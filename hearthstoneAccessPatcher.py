...
def get_hearthstone_dir_from_environment():
	process = subprocess.run(
		"powershell.exe [System.Environment]::GetEnvironmentVariable('HEARTHSTONE_HOME', 'User')", capture_output=True
	)
...
def move_patch_readme(hearthstone_dir):
	...
	want_readme = input('Type y for yes, or any other character and press enter.')
...
if __name__ == '__main__':
	...
	try:
		...
		print(
			'2. The patcher may not have privileges to modify files in the Hearthstone installation folder. Perhaps run it as an administrator.'
		)
	...
	try:
		...
		print('Here are some potential causes:')
		print(
			'1. The patcher may not have privileges to modify files in the Hearthstone installation folder. Perhaps run it as an administrator.'
		)
